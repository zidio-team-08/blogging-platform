...
export const getDashboardStats = async (req, res, next) => {
  try {
    ...
    if (recentUsersLimit && recentUsersLimit < 1)
      return next(
        new errorHandler("Recent users limit cannot be less than 1", 400)
      );

    if (recentUsersLimit && recentUsersLimit > 10)
      return next(
        new errorHandler("Recent users limit cannot be greater than 20", 400)
      );
...
export const getBlogs = async (req, res, next) => {
  try {
    ...
    if (limit < 3)
      return next(new errorHandler("Limit cannot be less than 3", 400));
